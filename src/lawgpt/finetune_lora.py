...
from lawgpt.utils.prompter import Prompter
...
def train(
    ...
    prompt_template_name: str = "roc_law",  # The prompt template to use, will default to roc_law.
):
    # Print info
    ...
    # Get pre-trained model from Hugging Face Models
    ...
    # Some tokenizers do not have pad_token_id
    if tokenizer.pad_token_id is None:
        tokenizer.pad_token_id = (
            tokenizer.unk_token_id
        )  # we want this to be different from the eos token
        # tokenizer.pad_token_id = 0  # 0==unk. we want this to be different from the eos token  # TODO: remove me
    # If they are the same, the loss of eos_token_id is not calculated when the model is trained
    ...
    model = prepare_model_for_int8_training(model)  # TODO: use prepare_model_for_kbit_training
